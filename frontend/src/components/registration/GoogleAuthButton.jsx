...
function GoogleAuthButtonInner({ role, onSuccess, onError, disabled }) {
  ...
  const login = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      setLoading(true);
      try {
        const api = role === 'student' ? studentAccountApi : tutorApi;
  ...
  return (
    <button
      type="button"
      className={`oauth-icon-btn${loading ? ' oauth-icon-btn--loading' : ''}`}
      onClick={() => !disabled && !loading && login()}
      disabled={disabled || loading}
      aria-label="Войти через Google"
      title="Google"
    >
      {loading ? <span className="oauth-icon-spinner" /> : <GoogleIcon />}
    </button>
  );
}
